...
class Live {
    ...
    async _loadPage(client) {
        ...
        // start the page load
        const navigation = Page.navigate({ url: this._url }).then(async () => {
            await new Promise((resolve) => {
                Page.loadEventFired(async () => {
                    const { root: { nodeId: documentId } } = await client.DOM.getDocument();
                    const { nodeId } = await client.DOM.querySelector({
                        nodeId: documentId,
                        selector: 'body'
                    });
                    const { model: { height } } = await client.DOM.getBoxModel({ nodeId });
